...
import { country_list, fetchCountryList, fetchStaffList } from '@/utils/apiUtils'
...
const selectedCatLabel = ref(null)
...
const fetchUsers = async () => {
  ...
  const res = await $api('/client/sorted-list', {
    ...
    body: {
      ...
      cat_label: selectedCatLabel.value,
    },
...
watch([page, itemsPerPage, selectedMTStatus, selectedDepositStatus, selectedDocStatus, selectedManager, selectedCountry, selectedIBStatus, selectedCatLabel, searchQuery, sortBy, orderBy], () => {
  fetchUsers()
    ...
    <VCard class="mb-6">
      ...
      <VCardText class="d-flex flex-wrap">
        <div class="me-3 d-flex gap-2">
          ...
          <!-- 👉 Select CAT LABEL -->
          <div style="inline-size: 15.625rem;">
            <AppSelect v-model="selectedCatLabel" label="Category Label" placeholder="Category Label"
                       :items="cat_label_list" clearable clear-icon="tabler-x"
            />
          </div>
        </div>
        <VSpacer/>
        ...
        <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
          <p class="card-subtitle pt-3">TOTAL LISTED CLIENTS : {{ totalUsers ? totalUsers : 0 }}</p>
          <!-- 👉 Add user button -->
          <VBtn v-if="hasAccess('add_new_client')" prepend-icon="tabler-plus"
