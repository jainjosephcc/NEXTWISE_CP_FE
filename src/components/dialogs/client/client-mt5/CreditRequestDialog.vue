...
const handleConfirm = async (confirmed) => {
  if (confirmed) {
    ...
    try {
      const res = await $api('/submitCreditInRequest', {
        method: 'POST',
        body: {
...
<template>
  ...
  >
    ...
    <VCard>
      <VCardTitle class="pt-3">REQUEST CREDIT</VCardTitle>
      <VDivider/>
      <VRow>
        <VCol cols="4">
          <VCardText>
      ...
      <VRow class="mt-2 ms-2 mb-3">
        ...
        <VCol cols="12">
          <VBtn class="me-2" color="primary" @click="handleSubmitBtn"
                :disabled="amtInUSD === null ||selectedOption===null " :loading="isSubmitLoading"
          >
            Submit
          </VBtn>
        </VCol>
  ...
  <ConfirmDialog v-model:isDialogVisible="isConfirmDialogVisible" @confirm="handleConfirm"
                 confirmation-question="Do you want to process this credit request?" cancel-title="Cancelled"
