...
watch(() => props.selectedTip, (newTip) => {
    if (newTip) {
        tipId.value=newTip.id;
        title.value = newTip.title || '';
        tip.value = newTip.tip || '';
        selectedLevel.value = newTip.level || '';
        selectedStatus.value = newTip.status || '';
 }
}, { immediate: true });

function handleEditDetailsSubmit() {

  if (tipId.value === null || title.value.trim() === '' || tip.value.trim() === '' || 
...
const handleConfirm = async (confirmed) => {
    if (confirmed) {
        ...
        try {
            const res = await $api('learning/update-forex-tip', {
                method: 'POST',
                body: {
                    id:tipId.value,
                    title:title.value,
            ...
            if (res) {
                ...
                successMessage.value = 'Trading Tip Updated Successfully!';
                isSubmitBtnLoading.value = false;
                isSubmitBtnDisable.value = false;
...
<template>
    <VDialog :width="$vuetify.display.smAndDown ? 'auto' : 800" :model-value="props.isDialogVisible"
        ...
        <VCard class="pa-6 pa-sm-10">
            ...
            <VForm @submit.prevent="() => { }">
                ...
                <VRow>
                    <VCol cols="6">
                        <VBtn class="me-2" color="primary" @click="handleEditDetailsSubmit()" :loading="isSubmitBtnLoading" :disabled="isSubmitBtnDisable">
                            Update 
                        </VBtn>
                    </VCol>
                </VRow>
    ...
    <ConfirmDialog v-model:isDialogVisible="isConfirmDialogVisible" @confirm="handleConfirm"
                confirmation-question="Are you sure you want to update the Trading Tip?"
