...
const existingTeamNames = ref(['Super Admin', 'Admin', 'Client Relationship Manager', 'Manager', 'Support Staff', 'Backend Team']);
const statusOptions = [
    { text: 'Active', value: 1 },
...
// Watch for changes in the selected role and update form fields
watch(() => props.selectedRole, (newValue) => {
    if (newValue) {
        roleid.value = newValue.id !== undefined ? newValue.id : '';
        rolename.value = newValue.role_name;
...
const isTeamNameTaken = computed(() => {
    return existingTeamNames.value.includes(rolename.value);
});

// Handle confirmation and submit form
const handleConfirm = async (confirmed) => {
    if (confirmed) {
        if (isTeamNameTaken.value) {
            errorTitle.value = 'Validation Error';
            errorMessage.value = 'The role name has already been taken.';
            isErrorDialogVisible.value = true;
            return;
        }
        isSubmitBtnLoading.value = true;
        try {
            const res = await $api('/EditStaffRoles', {
                method: 'POST',
                body: {
                    roleId: roleid.value,
        ...
        <VDialog :width="$vuetify.display.smAndDown ? 'auto' : 800" :model-value="props.isDialogVisible"
            ...
            <VCard class="pa-6 pa-sm-10">
                ...
                <VForm @submit.prevent="() => { }">
                    <VRow>
                        <VCol cols="12">
                            <AppTextField v-model="roleid" label="Role ID" disabled />
                        </VCol>
                        <VCol cols="12">
                            <AppTextField v-model="rolename" label="Role Name" :rules="[requiredValidator]"
                                :error-messages="isTeamNameTaken ? ['The role name has already been taken.'] : []" />
                        </VCol>
                        <VCol cols="12" md="12">
                ...
                <ConfirmDialog v-model:isDialogVisible="isConfirmDialogVisible" @confirm="handleConfirm"
                    confirmation-question="Are you sure you want to update the role?" cancel-title="Cancelled"
