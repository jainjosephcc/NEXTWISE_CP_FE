...
const handleConfirm = async (confirmed) => {
  if (confirmed) {
    ...
    try {
      const res = await $api('masters/store', {
        method: 'POST',
        body: {
          mc_name: copiername.value,
          server_id: props.serverDetails?.id,
...
<template>
  <section>
    <VDialog :width="$vuetify.display.smAndDown ? 'auto' : 650" :model-value="props.isDialogVisible"
      ...
      <VCard>
        <div class="sticky-card">
          <VCardTitle class="pt-3 ">Add Master Account</VCardTitle>
          <VDivider />
        </div>
        <VForm @submit.prevent="() => { }">
          ...
          <VRow class="mt-2 ml-1 mr-1">
            <VCol cols="6">
              <div>
                <AppTextField label="MT5 ID" v-model="searchQuery" placeholder="Search MT5 ID" />
              </div>
            </VCol>
            ...
            <VCol cols="6">
              <div class="">
                <AppTextField label="Description" v-model="description" />
              </div>
            </VCol>
    ...
    <ConfirmDialog v-model:isDialogVisible="isConfirmDialogVisible" @confirm="handleConfirm"
      confirmation-question="Are you want to add master?" cancel-title="Cancelled" confirm-title="Confirm" />
