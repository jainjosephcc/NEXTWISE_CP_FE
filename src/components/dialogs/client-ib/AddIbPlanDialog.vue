...
const isApproveBtnDisabled = ref(false);
    ...
    if (confirmed) {
        ...
        try {
            ...
            if (res) {
                ...
                successMessage.value = 'IB Plan Added Successfully !';
                isApproveBtnLoading.value = false;
...
<template>
    ...
    <VCard class="pa-6 pa-sm-10">
      ...
      <VForm @submit.prevent="() => {}">
        <VRow>
          <VCol cols="12">
            <AppTextField v-model="planname" label="Plan Name" :rules="[requiredValidator]"/>
          </VCol>
        </VRow>
        <VCol cols="6" class="mt-5">
            <VBtn class="me-2" :loading="isApproveBtnLoading" :disabled="isApproveBtnDisabled || planname===null" color="primary" @click="isConfirmDialogVisible = true">
                        Submit
            </VBtn>
        </VCol>
      </VForm>
    ...
    <ConfirmDialog v-model:isDialogVisible="isConfirmDialogVisible" @confirm="handleConfirm"
                confirmation-question="Are you want to add IB Plan?" cancel-title="Cancelled"
